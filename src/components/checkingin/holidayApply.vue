<template>
  <div class="holiday-apply">
    ...
    <div class="header_title">
      <a class="return_btn"
        @click="$router.back(-1)"><i class="zxicon zx-jt-left"></i>返回</a><span class="header_name">申请资料</span>
    </div>
    <div style="max-height: calc(100vh - 105px);">
      <el-form ref="zizhiform"
        :model="formData"
        :rules="rules"
        label-width="220px"
        style="width: 680px;padding-top:15px">
        ...
        <el-form-item label="开始时间"
          ...
          <el-col :span="11">
            <el-date-picker placeholder="请选择开始日期"
              ...
              v-model="formData.issuing_date1"></el-date-picker>
          </el-col>
          <el-col class="line"
            :span="2"
            style="text-align: center;">-</el-col>
          <el-col :span="11">
            <el-time-select placeholder="请选择开始时间"
      ...
      <el-form ref="approveform"
        ...
        <el-form-item label="审批人"
          prop="approver">
          <input type="hidden"
            :value="submitInfo.approver">
          <ul class="approver-icon clearfix">
            <li class="approver-item"
              v-if="submitInfo.approver"
              v-for="(item, index) in submitInfo.approver"
              :key="index">
            ...
            <li class="approver-item">
              <i class="zxicon zx-zengjia"
                @click="addApprover=true;"></i>
            </li>
          </ul>
        </el-form-item>
        ...
        <el-form-item label="抄送"
          prop="cclist">
          <input type="hidden"
            :value="submitInfo.cclist">
          <ul class="approver-icon clearfix">
            <li class="approver-item"
              v-if="submitInfo.cclist"
              v-for="(item, index) in submitInfo.cclist"
              :key="index">
              <img :src="item.imgUrl || defaultimg"
                width="40"
                height="40">
              <span class="btn-close"
                @click="removeCclist(item, index)"><i class="zxicon zx-guanbi"></i></span>
            </li>
            <li class="approver-item">
              <i class="zxicon zx-zengjia"
                @click="addCclist=true;"></i>
            </li>
          </ul>
        </el-form-item>
    ...
    <el-dialog title="添加审批人"
      :visible.sync="addApprover">
      <div class="add-approver">
        <cus-add-member :datas="datas"
          nameSpace='approver'
          :defaultimg='defaultimg'
          :render-after-expand="false"
          :defaultProps="defaultProps"
          :onlyChooseOne="true"
          @handle="addApproverHandle"></cus-add-member>
      </div>
      <div slot="footer"
        class="dialog-footer">
        <el-button @click="addApprover = false">取 消</el-button>
        <el-button type="primary"
          @click="addApprover = false;">确 定</el-button>
      </div>
    </el-dialog>

    <!-- 添加抄送人 -->
    <el-dialog title="添加抄送人"
      :visible.sync="addCclist">
      <div class="add-approver">
        <cus-add-member :datas="datas"
          nameSpace='cclist'
          :defaultimg='defaultimg'
          :render-after-expand="false"
          :defaultProps="defaultProps"
          @handle="addCclistHandle"></cus-add-member>
      </div>
      <div slot="footer"
        class="dialog-footer">
        <el-button @click="addCclist = false">取 消</el-button>
        <el-button type="primary"
          @click="addCclist = false;">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
