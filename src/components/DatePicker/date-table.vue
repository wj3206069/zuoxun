    ...
    <tbody>
      <tr>
        ...
        <th v-for="week in WEEKS"
            :key="'week'+week">{{ t('el.datepicker.weeks.' + week) }}</th>
      </tr>
...
export default {
        ...
        for (let j = 0; j < 7; j++) {
          ...
          if (i >= 0 && i <= 1) {
            if (j + i * 7 >= day + offset) {
              ...
              cell.status = this.getCellStatus(cell.text)
            } else {
              cell.text =
          ...
          cell.disabled =
            typeof disabledDate === 'function' && disabledDate(newDate)
          cell.selected =
  ...
  methods: {
    getCellStatus(text) {
      if (Array.isArray(this.cellStatus)) {
        let c = this.cellStatus.find(v => {
          if (v) {
            return v.text === text
          }
        })
    ...
    getCellClasses(cell) {
      ...
      if (cell.status) {
        classes.push(this.getTextClass(cell.status))
      }
      return classes.join(' ')
    },
    ...
    isWeekActive(cell) {
      ...
      return (
        year === valueYear &&
        getWeekNumber(newDate) === getWeekNumber(this.value)
      )
    },
