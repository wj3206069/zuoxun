  ...
  <el-main class="content_middle">
    ...
    <el-main>
      <p class="content_middle_title">
        ...
        <span>书签管理</span>
        <span class="icons">
             <el-button @click="addBookMarks('')" type="text" size="small">添加</el-button>
          </span>
      </p>
      <el-table :data="workData.list" tooltip-effect="dark" style="width: 100%" height="80%">
        ...
        <el-table-column prop="seat" label="位置">
             <template slot-scope="scope">
                <span v-if="scope.row.seat === '1'">书签栏</span>
                <span v-if="scope.row.seat === '2'">工具栏</span>
            </template>
        </el-table-column>
        <el-table-column prop="visible" label="类型">
             <template slot-scope="scope">
                <span v-if="scope.row.visible === '1'">公有</span>
                <span v-if="scope.row.visible === '2'">私有</span>
            </template>
        </el-table-column>
        <el-table-column prop="bigicon" label="大图标">
        </el-table-column>
        <el-table-column prop="smallicon" label="小图标">
        </el-table-column>
        <el-table-column
            label="操作"
            width="100">
            <template slot-scope="scope">
                <el-button @click="addBookMarks(scope.row.id)" type="text" size="small">修改</el-button>
    ...
    export default {
        ...
        methods: {
            addBookMarks(id) {
                ...
                this.$router.push("/bookBarksEdit/" + id);
            },
            getUserInfo() {
            ...
            loadWorkData() {
                ...
                this.$axios.get('/proxyapiBusiness/syncdata/bookMarks', {
                    params: {
            ...
            deleteData(data) {
                this.$axios.delete('/proxyapiBusiness/syncdata/bookMarks/' + data.id).then(res => {
                    if (res.success) {
