<template>
  <div class="cus-add-member">
    <div>
      <el-input size="small"
        placeholder="搜索"
        v-model="filterText"><i slot="prefix"
          class="zxicon zx-sousuo2"></i></el-input>
      <el-tree :ref="nameSpace"
        :props="defaultProps"
        ...
        <span class="cus-tree-node"
          ...
          <template>
            <span v-if="data.children && data.children.length > 0">
              ...
              <span v-else
                class="zxicon zx-wenjianjia9"></span>
            </span>
            <img v-else
              class="user-photo"
              :src="data.imgUrl || defaultimg"
              @error.once="handleImgError"
              alt="用户头像" />
          </template>
    ...
    <div>
      <ul class="tip">
        ...
        <li v-else>已选择<font class="num"
            color="#0cb200">{{ defaultMemberList.length }}</font>个成员</li>
      </ul>
      <ul class="checked-member-list">
        <li v-for="(item, index) in defaultMemberList"
          :key="index">
          ...
          <span v-if="!currentApp.iuserid || currentApp.iuserid != item.userid"
            class="btn-close"
            @click="removeMember(item, index)"><i class="zxicon zx-guanbi9"></i></span>
        </li>
      </ul>
    </div>
  </div>
