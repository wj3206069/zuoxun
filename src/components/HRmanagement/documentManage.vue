<template>
  <div class="zi-zhi-shenqing">
    ...
    <div class="header_title">
      ...
      <span class="header_name">证件信息</span>
    </div>
      ...
      <div class="HR-continater">
        <div class="register-info">
          <div class="register-info-content noborder basis-info">
            <el-form class="man-register" :model="row" :rules="rules" ref="documentInfoForm" label-width="110px" style="padding-top:15px;">
              <el-form-item class="man-stretch" label="名称" prop="zjmc">
                <el-input class="man-md" v-model="row.zjmc" placeholder="请输入证件名称"></el-input>
              </el-form-item>
              ...
              <el-form-item class="man-clearfix" label="编号" prop="zmbh">
                <el-input class="man-md" v-model="row.zmbh" placeholder="请输入编号"></el-input>
              </el-form-item>
    ...
    <div class="man-continater" style=" height: calc( 100% - 615px );min-height:230px">
      <BTable v-bind="tableDocument" @cellClick="cellClick" />
    </div>
...
import {
  ...
  certificateInformationAdd,
  certificateInformationEdit,
  certificateInformationDel,
  certificateInformationFiledel,
  fileUrl,
  delFile,
  downloadURL,
  download,
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        zjmc: [{ required: true, message: '不能为空', trigger: 'blur' }],
        type: [{ required: true, message: '不能为空', trigger: 'blur' }],
        bfdw: [{ required: true, message: '不能为空', trigger: 'blur' }],
        zmbh: [{ required: true, message: '不能为空', trigger: 'blur' }],
  ...
  methods: {
    ...
    search(){
      ...
      getCertificateInformation(query)
        .then(res => {
          this.tableDocument.list = res
          for (let i = 0; i < this.tableDocument.list.length; i++) {
            this.tableDocument.list[i].rowno = i + 1
          }
        })
        .catch(err => {
          console.log(err)
    ...
    saveInfo() {
      ...
      this.$refs['documentInfoForm'].validate(valid => {
        if (valid) {
            if (this.isAdd) {
              certificateInformationAdd(this.row)
                .then(res => {
                  this.init()
                  this.clearInfo()
                })
              ...
              certificateInformationEdit(this.row)
                .then(res => {
                  this.init()
                  this.clearInfo()
                })
    ...
    delInfo() {
      ...
      certificateInformationDel(query)
        .then(res => {
          if (res === 1) {
    ...
    beforeRemove(){
        // this.$confirm('此操作将永久删除该证件图片, 是否继续?', '提示', {
        //   confirmButtonText: '确定',
        //   cancelButtonText: '取消',
        //   type: 'warning'
        // })
    ...
    handleRemove(file, fileList) {
      ...
      delFile(query).then(res=>{
        ...
        if(this.row.id){
          certificateInformationFiledel(query).then(res=>{
            this.search()
          }).catch(err=>{
             console.log(err)
          })
